...
while IFS=$'\t' read -r label envs flags patchjson; do
  ...
  restore
  if [ "$patchjson" != "-" ]; then
    patch "$patchjson" || { echo "PATCH FAILED: $label"; continue; }
    node -e 'const a=require(process.argv[1]),b=JSON.parse(process.argv[2]);
      for(const k of Object.keys(b)) if(JSON.stringify(a[k])===undefined) process.exit(1);
      if (b.permissions && JSON.stringify(a.permissions.deny)!==JSON.stringify(b.permissions.deny)) process.exit(1);
      if ("includeGitInstructions" in b && a.includeGitInstructions!==b.includeGitInstructions) process.exit(1);' \
      "$SETTINGS" "$patchjson" || { echo "PATCH NOT APPLIED: $label"; restore; continue; }
  fi
  [ "$flags" = "-" ] && flags=""
  t=$(run "$envs" "$flags")
  restore
